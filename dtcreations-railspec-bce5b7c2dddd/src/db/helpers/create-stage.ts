...
type StageDefaults = Record<string, unknown>;
...
export async function createStageOnly(
  jobId: number,
  newStage: number
) {
  if (newStage <= 1) throw new Error("newStage must be > 1");
  const fromStage = newStage - 1;

  return db.transaction(async (tx) => {
    // 1) Ensure job exists & fetch job defaults
    ...
    // 2) Previous stage must exist
    const [prevStage] = await tx
      .select({ id: jobStages.id, defaults: jobStages.defaults })
      .from(jobStages)
      .where(and(eq(jobStages.jobId, jobId), eq(jobStages.stage, fromStage)))
      .limit(1);

    if (!prevStage) throw new Error(`Stage ${fromStage} not found`);

    // 3) New stage must not exist
    ...
    if (exists) throw new Error(`Stage ${newStage} already exists`);

    // 4) Decide defaults for the new stage:
    //    Prefer previous stage defaults; otherwise derive from job.
    ...
    const defaultsForNew =
      (prevDefaults && Object.keys(prevDefaults).length > 0)
        ? prevDefaults
        : buildDefaultsFromJob(jobRow);

    // 5) Create the stage (no balconies)
    ...
    return { jobId, newStage, newStageId: created.id };
  });
}
