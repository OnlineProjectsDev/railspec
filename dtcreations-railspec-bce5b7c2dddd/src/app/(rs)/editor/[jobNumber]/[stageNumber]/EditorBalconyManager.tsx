...
import { getBalustradeRunLengthMetersLabel } from "@/lib/balustrade/getBalustradeRunLengthMeters";
...
function getRunLengthFromPreviewRuns(runs?: { x: number; z: number }[][] | null): string {
  if (!runs?.length) return "0.0"

  const totalMm = runs.reduce((runTotal, run) => {
    if (!run || run.length < 2) return runTotal

    const runLength = run.slice(0, -1).reduce((segTotal, from, index) => {
      const to = run[index + 1]
      if (!to) return segTotal
      return segTotal + Math.hypot(to.x - from.x, to.z - from.z)
    }, 0)

    return runTotal + runLength
  }, 0)

  const meters = totalMm / 1000
  const rounded = Math.ceil(meters * 10) / 10
    ...
    <>
      ...
      {filteredBalconies.map((balcony, index) => {
        ...
        return (
          ...
          >
            <div className="w-32 h-20 shrink-0 rounded-md border bg-white overflow-hidden">
              <BalconyPreview
                balconyId={balcony.id}
                drop={balcony.drop}
                balconyNo={balcony.balconyNo}
                substrateOutline={balcony.substrateOutline}
                substrateEdges={balcony.substrateEdges}
                hasDerivedBalustrade={balcony.hasDerivedBalustrade}
                isFramelessSystem={balcony.isFramelessSystem}
                previewPosts={balcony.previewPosts}
                previewBalustradeRuns={balcony.previewBalustradeRuns}
              />
            </div>
            ...
            <div className="min-w-0 flex-1">
              {isEditing ? (
                ...
                <>
                  ...
                  <div className="text-[11px] text-muted-foreground mt-1">
                    Run length: {getRunLengthFromPreviewRuns(balcony.previewBalustradeRuns)}m
                  </div>
                  {balcony.notes ? (
