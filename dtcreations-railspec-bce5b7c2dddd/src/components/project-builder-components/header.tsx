...
interface HeaderProps {
  ...
  onReset: () => void;
  selectedData: insertJobSchemaType;
  viewMode: '2d' | '3d';
...
export default function Header({ steps, currentStep, onStepClick, onReset, selectedData, viewMode, onViewModeChange }: HeaderProps) {
  ...
  const canNavigateToStep = (targetStepId: number): boolean => {
    ...
    for (let stepId = 1; stepId < targetStepId; stepId++) {
      switch (stepId) {
        case 1:
          if (selectedData.design_default === null) return false;
          break;
        case 2:
          if (selectedData.anchorage_default === null) return false;
          break;
        case 3:
          if (selectedData.toprail_default === null) return false;
          break;
        case 4:
          if (selectedData.infill_default === null) return false;
          break;
      }
    }
    return true;
