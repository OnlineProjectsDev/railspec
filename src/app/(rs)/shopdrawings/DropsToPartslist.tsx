// /src/app/(rs)/shopdrawings/DropsToPartslist.tsx
...
// 🔹 Posts can carry structured drilling (P1..P4) and/or a preformatted string
export type PostFace = "P1" | "P2" | "P3" | "P4";


export type PostDrilling = Partial<Record<PostFace, number[]>> & {
  special?: Partial<Record<"SF" | "WF", Partial<Record<PostFace, number[]>>>>;
};

// Part types
export type PartTypePost = {id:number, post_id:number, partName:string, type:string, height:number, x: number, y:number, z: number, o:number, t:number, drilling?: PostDrilling}
export type PostVectorType = PlaneVec & {id:number}

export type PartTypeRail = {partName:string, id:number, post_id:number,  length: number, x:number,  y:number,  z:number,  o:number, t:number, lhc:number, rhc:number, lvc:number, rvc:number, drilling?: string}
export type PartTypeVertical = {id:number, post_id:number, partName:string, length:number, x: number, y:number, z: number, o:number, t:number, drilling?: string}

export type PartTypeGlass = {id:number, post_id:number, partName:string, length:number, height:number, thickness:number, infillType:string, x: number, y:number, z: number, o:number, t:number}
...
export type PartslistIdCounters = {
  toprailId: number;
  toprailVectorId: number;
};
...
export const DropToPartslist = (
  ...
  counters?: PartslistIdCounters
) => {
    // console.log("Glass0 " + infillType)
    ...
    const toprails = GetToprails(posts_array, xyo, design, toprail, counters);

    // console.log(posts_array)

    return {
        post_partslist: AddDrillingToPosts(post_partslist,midrail_partslist, vertical_infill_partslist, posts_array),
        ...
        // ✅ bubble these up
        nextToprailId: toprails.nextToprailId,
        nextToprailVectorId: toprails.nextToprailVectorId,

    }
}

// Decide which midrail types create post drilling points.
// (Tune this list as needed.)
function midrailContributesToPostDrilling(partName: string): boolean {
...
// Add this helper (similar to midrailContributesToPostDrilling)
function verticalContributesToPostDrilling(partName: string): boolean {
...
function sideFramePostDrillPoints(vLength: number): number[] {
  const mid = (vLength - 100) / 2;
  return [50, 50 + mid, 50 + mid + mid].map(round1);
}
// Decide opposing face based on the already-clamped angle [-180..180]
// P2 = 180 / -180 side
// P3 = +90 side
// P4 = -90 side
function opposingFaceFromAngle(angle: number): "P2" | "P3" | "P4" {
  // normalize just in case (though you said it's already clamped)
  let a = angle;
  if (a > 180) a -= 360;
  if (a <= -180) a += 360;

  // P2 near ±180
  if (a >= 125 || a <= -125) return "P2";

  // Remaining: decide between +90 vs -90
  // + angles -> P3, - angles -> P4
  return a >= 0 ? "P3" : "P4";
}


// Push unique integer (or fmt0) values into a face slot
function pushUniqueFace(drilling: PostDrilling, face: PostFace, y: number) {
  const slot = drilling[face] ?? [];
  if (!slot.includes(y)) slot.push(y);
  drilling[face] = slot;
}

function addTwoHeightsToFaces(
  drilling: PostDrilling,
  faces: [PostFace, PostFace],
  heights: number[]
) {
  for (const face of faces) {
    for (const h of heights) {
      pushUniqueFace(drilling, face, h);
    }
  }
}

// ---- YOU set these to the real values ----
const SFI_SFO_HEIGHTS = [/* e.g. */ 50, 200]; // <-- replace with your 2 specified heights

function sfHeightsForPost(post: PartTypePost): number[] {
  // WF “2 specified heights calculated from the 2 positions on the post”
  // Replace this with your real calculation.
  // Example placeholder: 2 holes at 100mm and 300mm from top:
  return [post.height-60-150, post.height-60];
}

function wfHeightsForPost(post: PartTypePost): number[] {
  // WF “2 specified heights calculated from the 2 positions on the post”
  // Replace this with your real calculation.
  // Example placeholder: 2 holes at 100mm and 300mm from top:
  return [100, post.height-100];
}


export function AddDrillingToPosts(
  posts_partslist: PartTypePost[],
  midrail_partslist: PartTypeRail[],
  vertical_partslist: PartTypeVertical[],   // ✅ NEW
  posts_array: PostsType[]                  // ✅ already planned/used
) {
  // Fast lookup: post_id -> all midrails that reference it
  const railsByPostId = new Map<number, PartTypeRail[]>();
  for (const r of midrail_partslist) {
    if (!Number.isFinite(r.post_id)) continue;
    const list = railsByPostId.get(r.post_id) ?? [];
    list.push(r);
    railsByPostId.set(r.post_id, list);
  }

  // ✅ NEW: Fast lookup: post_id -> all verticals that reference it
  const verticalsByPostId = new Map<number, PartTypeVertical[]>();
  for (const v of vertical_partslist) {
    if (!Number.isFinite(v.post_id)) continue;
    const list = verticalsByPostId.get(v.post_id) ?? [];
    list.push(v);
    verticalsByPostId.set(v.post_id, list);
  }

  const postIds = new Set(posts_partslist.map((p) => p.post_id));

  return posts_partslist.map((post, i) => {
    const drilling: PostDrilling = { ...(post.drilling ?? {}) };

    // -------------------------
    // MIDRAILS (your existing)
    // -------------------------

    // ✅ P1: rails fixed to this *post_id* (NOT local id)
    const faceSame = railsByPostId.get(post.post_id) ?? [];
    for (const r of faceSame) {
      if (!midrailContributesToPostDrilling(r.partName)) continue;

      const y = Math.round(post.height + post.y - midrailYDrilling(r.y, r.partName));
      const slot = drilling.P1 ?? [];
      pushUnique(slot, y);
      drilling.P1 = slot;
    }

    // ✅ Opposing face from previous *post_id* (global sequence)
    const prevId = post.post_id - 1;
    if (post.post_id > 0 && postIds.has(prevId)) {
      const facePrev = railsByPostId.get(prevId) ?? [];
      const faceKey = opposingFaceFromAngle(posts_array[i]?.angle ?? 0);

      for (const r of facePrev) {
        if (!midrailContributesToPostDrilling(r.partName)) continue;

        const y = Math.round(post.height + post.y - midrailYDrilling(r.y, r.partName));
        const slot = (drilling as any)[faceKey] ?? [];
        pushUnique(slot, y);
        (drilling as any)[faceKey] = slot;
      }
    }
    ...
    // ✅ P1: verticals fixed to this *post_id*
    const verticalSame = verticalsByPostId.get(post.post_id) ?? [];
    for (const v of verticalSame) {
      if (!verticalContributesToPostDrilling(v.partName)) continue;

      const pts = sideFramePostDrillPoints(v.length);
      const slot = drilling.P1 ?? [];
      for (const y of pts) pushUnique(slot, y);
      drilling.P1 = slot;
    }

    // ✅ Opposing face from previous *post_id*
    if (post.post_id > 0 && postIds.has(prevId)) {
      const verticalPrev = verticalsByPostId.get(prevId) ?? [];
      const faceKey = opposingFaceFromAngle(posts_array[i]?.angle ?? 0);

      for (const v of verticalPrev) {
        if (!verticalContributesToPostDrilling(v.partName)) continue;
    ...
    // -------------------------
    // POST TYPE DRILLING (SFI/SFO/WF)  <-- ADD HERE
    // -------------------------
    // Uses: post.type, posts_array[i].reversed, posts_array[i].angle,
    //       and checks whether faces already have holes from midrails/verticals.   

     // ----------------------------
    // Post-type drilling rules
    // ----------------------------
    const src = posts_array[i]; // relies on same ordering

    // Guard if arrays are out of sync
    if (src) {
    const reversed = !!src.reversed;

    // Helpers to detect if midrail/vertical already created drilling on a face (NORMAL drilling only)
    const hasP1 = (drilling.P1?.length ?? 0) > 0;
    const hasP2 = (drilling.P2?.length ?? 0) > 0;
    const hasP3 = ((drilling as any).P3?.length ?? 0) > 0;
    const hasP4 = ((drilling as any).P4?.length ?? 0) > 0;

    const hasBefore = hasP2 || hasP3 || hasP4; // "before the post" = opposing faces
    const hasAfter = hasP1;                    // "after the post"  = P1 face
    ...
    // - normal: P3 + P4
    // - reversed: P1 + P2
    if (post.type === "SFI" || post.type === "SFO") {
        const faces: [PostFace, PostFace] = reversed ? ["P1", "P2"] : ["P3", "P4"];
        const sfHeights = sfHeightsForPost(post).map((n) => Math.round(n * 10) / 10);
        for (const face of faces) {
        for (const h of sfHeights) {
            pushUniqueSpecial(drilling, "SF", face, Math.round(h * 10) / 10); // fmt1-ish storage
        }
        }
    }
    ...
    // ----------------------------
    // Faces:
    // - WF on P1 if there is NO midrail/vertical "before" (no P2/P3/P4 normal drilling)
    // - WF on P2 if there is NO midrail/vertical "after"  (no P1 normal drilling)
    if (post.type === "WF") {
        const wfHeights = wfHeightsForPost(post).map((n) => Math.round(n * 10) / 10);

        if (!hasBefore) {
        for (const h of wfHeights) pushUniqueSpecial(drilling, "WF", reversed ? "P3" : "P1", h);
        } else if (!hasAfter) {
        for (const h of wfHeights) pushUniqueSpecial(drilling, "WF", reversed ? "P3" :  "P2", h);
        }
    }
    }


    // Optional stability/readability
    if (drilling.P1?.length) drilling.P1.sort((a, b) => a - b);
    if (drilling.P2?.length) drilling.P2.sort((a, b) => a - b);
    if ((drilling as any).P3?.length) (drilling as any).P3.sort((a: number, b: number) => a - b);
    if ((drilling as any).P4?.length) (drilling as any).P4.sort((a: number, b: number) => a - b);

    const changed =
      (drilling.P1?.length ?? 0) > 0 ||
      (drilling.P2?.length ?? 0) > 0 ||
      (((drilling as any).P3?.length ?? 0) > 0) ||
      (((drilling as any).P4?.length ?? 0) > 0);

    //   console.log(post.post_id, drilling)

    return changed ? { ...post, drilling } : post;
  });
}
...
function GetMidrails(posts_array: PostsType[], infill_vectors_array: InfillVectorType[], xyo: { x: number; y: number; o: number} [], design: string, infillType: string){
    ...
    let drilling_info = null;

    if (design === "RD-D3" || design === "RD-D4"){
                // Add bottom Balusters
                let spacing = 120

                const quantity = Math.ceil((length-spacing+19)/spacing)
                // console.log(quantity)
                if(infillType == "Balusters Equally Spaced"){
                    spacing = Round((length+19)/(quantity+1),2)
                }

                const first = Round((length - spacing*((quantity-1))-13)/2,1)
                drilling_info = `X${first} S13 C${Round(spacing,1)} N${quantity}`
            ...
            if (design === "RD-D3SLATS" || design === "RD-D4SLATS"){
                // Add Slats
                let spacing = 165
                const quantity = Math.ceil((length-spacing+65)/spacing)
                if(infillType == "Slats Equally Spaced"){
                    spacing = Round((length+65)/(quantity+1),2)
                }

                const first = Round((length - spacing*((quantity-1))-13)/2,1)
                drilling_info = `X${first} S55 C${Round(spacing,1)} N${quantity}`
    ...
    if (design === "RD-D1" || design === "RD-D2" || design === "RD-D7" || design === "RD-D8"){
            ...
            if (design === "RD-D3" || design === "RD-D4" || design === "RD-D3SLATS" || design === "RD-D4SLATS"){
                ...
                midrail_partslist.push({
                    ...
                    lvc: 90,
                    rvc: 90,
                    drilling: drilling_info ?? undefined
                })
            }
        ...
        if ((POST_FIXING_TYPES.includes(posts_array[i].type) || posts_array[i].type === "WF") && (POST_FIXING_TYPES.includes(posts_array[i+1].type) || posts_array[i+1].type === "WF")){
            ...
            if (design === "RD-D5"){
                ...
                vertical_infill_partslist.push({
                    ...
                    lvc: 0,
                    rvc: 0,
                    drilling: sideFramePostDrillPoints(length).map(fmt0).join(" _ ")
...
function GetGlassPanels(posts_array:PostsType[], infill_vectors_array:InfillVectorType[], xyo: {x:number, y:number, o:number}[],design:string, infillType:string){
    let glass_infill_partslist: PartTypeGlass[] = []

    console.log(infill_vectors_array.length)
        ...
        if ((POST_FIXING_TYPES.includes(posts_array[i].type) || posts_array[i].type === "WF" || SPIGOT_TYPES.includes(posts_array[i].type)) && (POST_FIXING_TYPES.includes(posts_array[i+1].type) || posts_array[i+1].type === "WF" || SPIGOT_TYPES.includes(posts_array[i+1].type))){
            ...
            if (design === "RD-D1" || design === "RD-D2" || design === "RD-D7" || design === "RD-D8"){
                ...
                glass_infill_partslist.push({
                    ...
                    height: Round(infill_vector.top.y-infill_vector.bottom.y+9,0),
...
function GetToprails(
  posts_array: PostsType[],
  xyo: { x: number; y: number; o: number }[],
  design: string,
  toprail: string,
  counters?: PartslistIdCounters
) {
  ...
  // ✅ global counters (persist across balconies)
    const nextToprailId = () => {
    if (counters) return counters.toprailId++;
    return toprail_partslist.length ?? 0; // fallback (existing behaviour)
    };

    const nextToprailVectorId = () => {
    if (counters) return counters.toprailVectorId++;
    return toprail_vectorslist.length ?? 0; // fallback (existing behaviour)
    };
    
  if (toprail !== "None"){

const pushToprail = (post: any, oVal: number) => {
  // ✅ Plane directions (use only v_x/v_z like midrails)
  const planeL = { x: left_cut.v_x, y: 0, z: left_cut.v_z };
  const planeR = { x: right_cut.v_x, y: 0, z: right_cut.v_z };

  // ✅ Rail direction from ACTUAL cut points (this is the missing piece)
  const railLR = normalize(
    right_cut.x - left_cut.x,
    0,
    right_cut.z - left_cut.z
  ).vector;

  const railRL = { x: -railLR.x, y: 0, z: -railLR.z };

  // ✅ Same convention that produced correct midrails:
  // LHC: rail -> plane (left towards center)
  // RHC: plane -> rail (right towards center)
  const thetaL = signedAngleXZ(railLR, planeL);
  const thetaR = signedAngleXZ(planeR, railRL);

  const lhc = Round(fold0_180(90 - thetaL),1);
  const rhc = Round(fold0_180(90 - thetaR),1);

  toprail_partslist.push({
    partName: toprail_name + " Toprail",
    id: nextToprailId(),
    post_id: post.id,
    length: Round(distance(left_cut,right_cut) + 0.5*55*(cotDeg(lhc)+cotDeg(rhc)), 2),
...
//   console.log("Toprail lhc/rhc (display):", Round(lhc, 3), Round(rhc, 3), Round(distance(left_cut,right_cut), 2), Round(distance(left_cut,right_cut) + 0.5*55*(cotDeg(lhc)+cotDeg(rhc)), 2));
};
  ...
  posts_array.map((post, i) => {
    if (i === 0) {
      ...
      i === posts_array.length - 1 ||
      ((posts_array[i - 1].type != "EC" && posts_array[i - 1].type != "WC" && posts_array[i - 1].type != "S") &&
        (post.type === "EC" || post.type === "WC"))
    ) {
      const d_vector = angleDegToXZVector(xyo[i].o);
      ...
      right_cut = {
        ...
        // ✅ last toprail code
        sectionEndI = i;        // piece follows xyo[sectionStartI] -> xyo[i]
        pushToprail(post, xyo[i].o);
        sectionStartI = sectionEndI;
      ...
      if (Math.abs(post.angle) !== 180) {
        ...
        sectionEndI = i - 1;    // that piece follows segment i-1
        pushToprail(post, xyo[i - 1].o);
        sectionStartI = sectionEndI;
        ...
        right_cut = {
          x: post.x + 22.5 * d_vector.x,
          y: post.y_ref2 + 22.5 * d_vector.y,
          z: post.z + 22.5 * d_vector.z,
          v_x: -d_vector.x,
          v_y: -d_vector.y,
          v_z: -d_vector.z,
        };
        ...
        sectionEndI = i;        // split along current segment i
        pushToprail(post, xyo[i].o);
        sectionStartI = sectionEndI;
...
function computeCutsFromXyoAndPlanes_NoSnap(
  xyo0: { x:number; y:number; o:number },
  xyo1: { x:number; y:number; o:number },
  planeL: { x:number; y:number; z:number },
  planeR: { x:number; y:number; z:number },
) {
  // Rail direction = authoritative xyo segment direction
  const railLR = normalize(xyo1.x - xyo0.x, 0, xyo1.y - xyo0.y).vector;
  const railRL = { x: -railLR.x, y: 0, z: -railLR.z };

  // Same convention as your validated midrails
  const thetaL = signedAngleXZ(railLR, planeL);
  const thetaR = signedAngleXZ(planeR, railRL);

  const lhc = fold0_180(90 - thetaL);
  const rhc = fold0_180(90 - thetaR);

  return { lhc, rhc, thetaL, thetaR, railLR, railRL };
}
...
function normalize(x:number,y:number,z:number){
  ...
  //  console.log(norm);
  return {vector:{x:(x/norm), y:(y/norm), z:(z/norm)},norm:norm};
}

function normalizeV(v:{x:number,y:number,z:number}){
  let norm = norm_function(v.x,v.y,v.z);
  //  console.log(norm);
  return {vector:{x:(v.x/norm), y:(v.y/norm), z:(v.z/norm)},norm:norm};
}
...
/**
 * Full “lhc/rhc” cut computation for a segment:
 * - railLR: left -> right
 * - railRL: right -> left (into the rail from the right)
 */
// Use the plane "direction" from v_x/v_z (NOT the point's x/z position)
function planeDirXZ(p: { v_x?: number | null; v_z?: number | null }) {
  return { x: p.v_x ?? 0, y: 0, z: p.v_z ?? 0 };
}

// Normalize and compute cut towards rail centre.
// Convention: signedAngle is plane→rail (so negative means rail is clockwise from plane in XZ).
function cutAngleFromPlaneAndRail_Flat(
  plane: { v_x?: number | null; v_z?: number | null },
  railDir: { x: number; y: number; z: number }
) {
  const pDir = planeDirXZ(plane);
  const a = signedAngleXZ(pDir, railDir); // plane → rail  :contentReference[oaicite:2]{index=2}
  // IMPORTANT: cut is 90 - signedAngle(plane→rail)
  return fold0_180(90 - a);
}

// Keep result in [0, 180]
...
function midrailMitresFromPlanesAndXyo(
  xyo0: {x:number,y:number,o:number},
  xyo1: {x:number,y:number,o:number},
  left:  {v_x:number,v_z:number},
  right: {v_x:number,v_z:number},
) {
  const railLR = normalize(xyo1.x - xyo0.x, 0, xyo1.y - xyo0.y).vector;
  const railRL = { x: -railLR.x, y: 0, z: -railLR.z };

  const planeL = { x: left.v_x,  y: 0, z: left.v_z };
  const planeR = { x: right.v_x, y: 0, z: right.v_z };
...
function normalizeThetaMagnitudeDeg(thetaDeg: number) {
  // map to [0, 180]
  let x = ((thetaDeg % 360) + 360) % 360; // [0, 360)
  if (x > 180) x = 360 - x;
  return x; // [0, 180]
}
