// /app/(rs)/fabrication/raw/[jobNumber]/[stageNumber]/powdercoat/page.tsx
...
import StagePowdercoatOrderTable from "./StagePowdercoatOrderTable"
...
export default async function FabricationPowdercoatPage({ params }: PageProps) {
  try {
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">
            Job #{job.job_number} — Stage {jobStage.stage} Powdercoat Order
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Powdercoated extrusion stock requirements with cut planning, plus non-length powdercoated items.
          </p>
        </div>
          ...
          <div className="rounded-md border p-4 bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <div>Colour: {powdercoatColour?.name ?? "TBD"}</div>
              </div>
            </div>
          </div>

          <div className="rounded-md border p-4 bg-white">
            <h2 className="text-lg font-semibold mb-3">Powdercoat Summary</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div className="rounded-md border p-3">
                <div className="text-muted-foreground">Length Rows</div>
                <div className="text-xl font-semibold">{totalLengthRows}</div>
              </div>
              <div className="rounded-md border p-3">
                <div className="text-muted-foreground">Non-length Rows</div>
                <div className="text-xl font-semibold">{totalNonLengthRows}</div>
              </div>
              <div className="rounded-md border p-3">
                <div className="text-muted-foreground">Profiles Planned</div>
                <div className="text-xl font-semibold">{powdercoat.cutPlans.length}</div>
              </div>
              <div className="rounded-md border p-3">
                <div className="text-muted-foreground">Bars to Order</div>
                <div className="text-xl font-semibold">{totalPlannedBars}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-md border p-4 bg-white">
          <div className="flex items-center justify-between gap-4 mb-3">
            <h2 className="text-lg font-semibold">Powdercoat Order</h2>
            <div className="text-sm text-muted-foreground">
              {powdercoat.cutPlans.length} planned profiles
            </div>
          </div>
